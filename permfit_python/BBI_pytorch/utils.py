...
def joblib_ensemble_dnnet(
    ...
    for j in range(n_layer):
        if j == 0:
            pred = relu(
                X_scaled_n.dot(current_model[0][j]) + current_model[1][j]
            )
        else:
            pred = relu(
                pred.dot(current_model[0][j]) + current_model[1][j]
            )
    ...
    if prob_type != "classification":
        ...
        loss = np.std(y_valid) ** 2 - mean_squared_error(
            y_valid, pred_v[valid_ind]
        )
    else:
        pred_v = link_func[prob_type](pred)
        loss = log_loss(
...
class DNN(nn.Module):
    ...
    def forward(self, x):
        if self.group_stacking:
            ...
            for ind_layer, layer in enumerate(self.layers_stacking):
                list_stacking[ind_layer] = layer(
                    x[:, self.list_grps[ind_layer]]
                )
    ...
    def validation_epoch_end(self, outputs, prob_type):
        if prob_type == "classification":
            ...
            batch_losses = [
                x["val_mse"] * x["batch_size"] for x in outputs
            ]
...
def evaluate(model, loader, device, prob_type):
    outputs = [
        model.validation_step(batch, device, prob_type) for batch in loader
    ]
...
def dnn_net(
    ...
    validate_loader = Dataset_Loader(
        X_valid, y_valid, batch_size=batch_size_val
    )
    ...
    for epoch in range(n_epoch):
        ...
        for batch in train_loader:
            ...
            for name, param in model.named_parameters():
                if "bias" not in name:
                    param.data -= (
                        l1_weight * torch.sign(param.data)
                        + l2_weight * param.data
                    )
        # Validation Phase
        model.eval()
    ...
    for name, param in dict_params.items():
        ...
        if name.split(".")[0] == "layers_stacking":
            ...
            if name.split(".")[-1] == "bias":
                best_bias_stack[curr_ind].append(
                    param.numpy()[np.newaxis, :]
                )
