...
class DNN_learner_single(BaseEstimator):
    ...
    def fit(self, X, y=None):
        ...
        for i in range(self.n_ensemble - 1):
            current_pred = np.mean(
                pred_m[loss >= sorted_loss[i], :], axis=0
            )
            if self.prob_type == "regression":
        ...
        keep_dnn = (
            ...
            >= sorted_loss[
                np.argmin(
                    new_loss[: (self.n_ensemble - self.min_keep + 1)]
                )
            ]
        )
    ...
    def __encode_outcome(self, y, train=True):
        ...
        for col in range(y.shape[1]):
            if train:
                ...
                if self.prob_type == "classification":
                    list_y.append(
                        self.enc_y[col].transform(y[:, [col]]).toarray()
                    )
    ...
    def __scale_test(self, X):
        ...
        if self.group_stacking:
            ...
            for mod in range(len(self.optimal_list)):
                ...
                X_test_scaled[:, self.list_cont] = self.optimal_list[mod][
                    1
                ][0].transform(X_test_scaled[:, self.list_cont])
                ...
                for grp_ind in range(len(self.list_grps)):
                    curr_pred = X_test_scaled[
                        :, self.list_grps[grp_ind]
                    ].copy()
                    n_layer_stacking = (
                        len(self.optimal_list[mod][0][3][grp_ind]) - 1
                    )
            ...
            for j in range(n_layer):
                if not self.group_stacking:
                    if len(self.list_cont) > 0:
                        X_test_scaled[:, self.list_cont] = mod[1][
                            0
                        ].transform(X_test_scaled[:, self.list_cont])
                if j == 0:
                    pred = relu(
                        X_test_scaled.dot(mod[0][0][j]) + mod[0][1][j]
                    )
                else:
                    pred = relu(pred.dot(mod[0][0][j]) + mod[0][1][j])

            self.pred[ind_mod] = (
                pred.dot(mod[0][0][n_layer]) + mod[0][1][n_layer]
            )
