...
class DNN_learner(BaseEstimator):
    ...
    def hyper_tuning(
        ...
        #     list_res.append(estimator.hyper_tuning(X_train, y_train, X_valid, y_valid, random_state))
        # print(list_res)
        # exit(0)
    ...
    def predict(self, X, scale=True):
        ...
        for estimator_ind, estimator in enumerate(self.list_estimators):
            if isinstance(X, list):
                ...
                list_res.append(
                    estimator.predict(X[estimator_ind, ...], scale)
                )
    ...
    def predict_proba(self, X, scale=True):
        ...
        for estimator_ind, estimator in enumerate(self.list_estimators):
            if isinstance(X, list):
                ...
                list_res.append(
                    estimator.predict_proba(X[estimator_ind, ...], scale)
                )
                self.X_test[estimator_ind] = estimator.X_test.copy()
