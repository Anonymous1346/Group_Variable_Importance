...
from .utils import create_X_y, convert_predict_proba
import warnings


class BlockBasedImportance(BaseEstimator, TransformerMixin):
    """This class implements the Block Based Importance (BBI), it consists of
        the learner block (first block) and the importance block (second block).
    Parameters
    ----------
    estimator: scikit-learn compatible estimator, default=None
        The provided estimator for the prediction task (First block).
    importance_estimator: {scikit-learn compatible estimator or string}, default=None
    ...
    def fit(self, X, y=None):
        ...
        self : object
            Returns self.
        """
        # Remove the group stacking option for non-DNN estimators
        if self.estimator is not None:
            self.group_stacking = False
            warnings.warn(
                "The stacking method is currently not implemented for non-DNN learners."
            )

        # Fixing the random generator's seed
        self.rng = np.random.RandomState(self.random_state)

        # Convert list_nominal to a dictionary if initialized as an empty string
        ...
        if (self.dict_hyper is None) and (self.estimator is None):
            self.dict_hyper = {
                "lr": [1e-4, 1e-3, 1e-2],
        ...
        if (self.groups is None) or (not bool(self.groups)):
            # Initialize the list_cols variable with each feature in a seperate list (default case)
            self.groups = [[col] for col in X.columns]
            if self.group_stacking:
                # Remove the group_stacking flag not to complex the DNN architecture
                self.group_stacking = False
                warnings.warn(
                    "The groups are not provided to apply the stacking approach, "
        ...
        # Initialize the estimator (block learner)
    ...
    def compute_importance(self, X=None, y=None):
        ...
        # Compute the standard deviation of each fold over the number of observations
        pred_std = np.array(
            [
                np.mean(
