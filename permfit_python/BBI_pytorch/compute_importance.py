...
def joblib_compute_conditional(
    ...
    grp_ord = [
        item
        for item in p_col
        if (item in dict_nom.keys()) and (item not in grp_nom)
    ]
    ...
    if importance_estimator != "Mod_RF":
        if var_type["regression"]:
            for counter_test, X_test_comp in enumerate(
                current_X_test_list
            ):
                ...
                X_col_pred["regression"][counter_test] = X_col_pred[
                    "regression"
                ][counter_test].reshape(output["regression"].shape)
        ...
        if var_type["ordinal"]:
            ...
            output["ordinal"] = ordinal_encode(
                np.array(X_nominal[grp_ord])
            )
            X_col_pred["ordinal"] = []
            for cur_output_ind in range(X_test_minus_idx.shape[0]):
                current_prediction = []
                for cur_ordinal in output["ordinal"]:
                    ...
                    current_prediction.append(
                        convert_predict_proba(np.array(probs))
                    )
        ...
        for counter_test, X_test_comp in enumerate(current_X_test_list):
            if var_type["regression"]:
                ...
                for cur_output_ind in range(X_test_minus_idx.shape[0]):
                    ...
                    pred_mod_RF["regression"][
                        counter_test
                    ] = importance_models["regression"].sample_same_leaf(
                        X_test_minus_idx[cur_output_ind, ...]
                    )
        ...
        if var_type["ordinal"]:
            ...
            for cur_output_ind in range(X_test_minus_idx.shape[0]):
                for cur_ordinal_ind, cur_ordinal in enumerate(
                    output["ordinal"]
                ):
                    importance_models["ordinal"] = hypertune_predictor(
                        importance_models["ordinal"],
                    ...
                    pred_mod_RF["ordinal"][
                        cur_ordinal_ind
                    ] = importance_models["ordinal"].sample_same_leaf(
                        X_test_minus_idx[cur_output_ind, ...],
    ...
    for sample in range(n_sample):
            ...
            for counter_test, X_test_comp in enumerate(
                ...
                if var_type["classification"]:
                    ...
                    for cur_output in X_col_pred["classification"]:
                        list_seed_cat = rng.randint(
                            1e5, size=len(cur_output)
                        )
                        for cat_prob_ind in range(len(cur_output)):
                            rng_cat = np.random.RandomState(
                                list_seed_cat[cat_prob_ind]
                            )
                ...
                if var_type["ordinal"]:
                    for cur_output in X_col_pred["ordinal"]:
                        list_seed_ord = rng.randint(
                            1e5, size=len(cur_output)
                        )
                        for ord_prob_ind, ord_prob in enumerate(
                            cur_output
                        ):
                            rng_ord = np.random.RandomState(
                                list_seed_ord[ord_prob_ind]
                            )
                            current_X_col_new = [None] * ord_prob.shape[0]
                            for i in range(ord_prob.shape[0]):
                                current_sample = [0] * ord_prob.shape[1]
                                for ind_prob, prob in enumerate(
                                    ord_prob[i]
                                ):
                                    current_sample[
                                        ind_prob
                                ...
                                #         break
                                ind_cat = np.where(
                                    current_X_col_new[i] == 1
                                )[0]
                                current_X_col_new[i] = [
                                    np.unique(
                                        X_nominal[grp_ord[ord_prob_ind]]
                                    )[len(ind_cat)]
                                ]
                    ...
                    X_test_comp[..., p_col_n["ordinal"]] = X_col_new[
                        "ordinal"
                    ]
        else:
            for counter_test, X_test_comp in enumerate(
                ...
                if var_type["classification"]:
                    ...
                    for cat_prob_ind in range(predictions.shape[-1]):
                        class_col = np.array(
                            [
                                [max_val(row)]
                                for row in predictions[..., cat_prob_ind]
                            ]
                        )
                        if grp_nom[cat_prob_ind] in encoder:
                            current_X_col_new = (
                ...
                if var_type["ordinal"]:
                    max_val = lambda row: list(Counter(row).keys())[0]
                    for cur_output_ind in range(
                        len(pred_mod_RF["ordinal"])
                    ):
                        predictions = sample_predictions(
        ...
        if prob_type == "regression":
            if type_predictor == "DNN":
                pred_i = estimator.predict(
                    current_X_test_list, scale=False
                )
            else:
                pred_i = estimator.predict(
                    current_X_test_list[0].squeeze()
                )
            ...
            res_ar[sample, ::] = (y_test - pred_i) ** 2 - (
                y_test - org_pred
            ) ** 2
        else:
            if type_predictor == "DNN":
                pred_i = estimator.predict_proba(
                    current_X_test_list, scale=False
                )
            else:
            ...
            res_ar[sample, :, 0] = -np.sum(
                y_test * np.log(pred_i), axis=1
            ) + np.sum(y_test * np.log(org_pred), axis=1)
...
def joblib_compute_permutation(
    ...
    for X_test_comp in current_X_test_list:
        X_test_comp[..., p_col_new] = X_test_comp[..., p_col_new].take(
            indices, axis=-2
        )
